...
(def a-6-4 "select irstream symbol, volume,  sum(price) from MarketData.win:time(5.50 sec) group by symbol output last every 1 seconds order by symbol")
...
;; help
;; TODO: need more clean up on symbols: market-data-property, k, a-6-4
(println " ")
(println " ")
