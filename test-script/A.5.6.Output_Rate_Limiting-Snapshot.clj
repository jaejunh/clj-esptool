...
(def a-5-6 "select irstream symbol, sum(price) from MarketData.win:time(5.50 sec) group by symbol output snapshot every 1 seconds order by symbol")
...
;; help
;; TODO: need more clean up on symbols: market-data-property, k, a-5-6
(println " ")
(println " ")
